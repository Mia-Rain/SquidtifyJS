...
/*TOKEN GEN*/ $.ajax({
 ...
 //  crossDomain: true, // using this header only causes problems.. 
  ...
  data: {
    ...
    client_secret: CLSEC,
    redirect_uri:
      "https://thatgeekyweeb.github.io/SquidtifyJS/callback/",
    code,
  ...
  error: function () {
    ...
    swal({
      ...
      text:
        "There was a error in requesting the Refresh Token... We need to restart...",
      icon: "warning",
      buttons: true,
      dangerMode: true,
    }).then((reload) => {
      if (reload) {
        window.location.replace(
          "https://thatgeekyweeb.github.io/SquidtifyJS/"
        );
      } else {
        swal.close();
      }
...
// WP-SWITCHER
wps = [
  "https://github.com/ThatGeekyWeeb/Squid-Dots/raw/master/wallpapers/shift.png",
  "https://github.com/ThatGeekyWeeb/Squid-Dots/raw/master/wallpapers/spin.png",
  "https://github.com/ThatGeekyWeeb/Squid-Dots/raw/master/wallpapers/water.png",
  "https://github.com/ThatGeekyWeeb/Squid-Dots/raw/master/wallpapers/the-blood-is-there.png",
  "https://github.com/ThatGeekyWeeb/Squid-Dots/raw/master/wallpapers/im-crying.png",
  "https://github.com/ThatGeekyWeeb/Squid-Dots/raw/master/wallpapers/blended.png",
  "https://github.com/ThatGeekyWeeb/Squid-Dots/raw/master/wallpapers/clouds.png",
];
...
function wpRight() {
  ...
  document.body.style.background =
    "url(" + wps[parseInt(sessionStorage.getItem("num"))] + ")";
}
function wpLeft() {
  ...
  if (sessionStorage.getItem("num") === null) {
    ...
    sessionStorage.getItem("num") == "NaN"
  ) {
    document.body.style.background = null;
...
function li() {
  if (document.querySelectorAll("[class=player]")[0].style.display == "none") {
    document.querySelectorAll("[class=player]")[0].style.display = null;
  } else {
    document.querySelectorAll("[class=player]")[0].style.display = "none";
  }
  if (document.getElementById("li").style.display == "none") {
    document.getElementById("li").style.display = null;
  } else {
    document
      .getElementById("li")
...
window.onSpotifyWebPlaybackSDKReady = () => {
  ...
  const player = new Spotify.Player({
    ...
    getOAuthToken: (callback) => {
      callback(sessionStorage.getItem("access_token"));
    },
  });
  ...
  player.addListener("initialization_error", ({ message }) => {
    ...
    sessionStorage.setItem("HDRM", undefined);
  });
  player.addListener("authentication_error", ({ message }) => {
    console.error(message);
    console.log("ERROR IN PLAYER AUTH");
  }); //window.location.replace('https://thatgeekyweeb.github.io/SquidtifyJS/'); });
...
setInterval(function () {
  ...
  $.ajax({
    ...
    success: function (response, data) {
      if (
        sessionStorage.getItem("device") != undefined &&
        response != "" &&
        response != undefined &&
        response.shuffle_state == true
      ) {
        ...
        sessionStorage.getItem("device") == "not found" ||
        sessionStorage.getItem("device") === null
      ) {
        swal({
          icon: "error",
  ...
  $.ajax({
    ...
    success: function (response, data) {
      document.querySelectorAll("[class=slider_container] input")[1].value =
        response.device.volume_percent; // Sets volume slider to current state
    },
  });
  $.ajax({
    type: "GET",
  ...
  if (
    uri != undefined &&
    uri != "" &&
    sessionStorage.getItem("uri") != undefined &&
    ...
    $.ajax({
      ...
      success: function (response, data) {
        if (
          ...
          response.album.images[0].url != undefined
        ) {
          ...
          swal({
            ...
            if (`${value}` == "true") {
              ...
              console.error(
                "Resume was refussed, you should reload the page, as it is broken now... Addtionally remeber to start playback"
              );
            }
          });
        }
  ...
  $.ajax({
    ...
    success: function (response, data) {
      if (
        ...
        if (
          parseInt(document.querySelector("input[type=range]").value, 10) ==
            1 ||
          parseInt(document.querySelector("input[type=range]").value, 10) >= 90
        ) {
          document.querySelector("input[type=range]").value = Math.floor(
        ...
        $.ajax({
          ...
          success: function (response, data) {
            if (
              ...
              document.getElementById("durr").innerHTML = mills(
                response.duration_ms
              );
            } else {
              document.getElementById("durr").innerHTML = "Unset";
            }
          },
        });
      }
  ...
  $.ajax({
    ...
    success: function (response, data) {
      if (
        ...
        response.item.artists[0].name != undefined
      ) {
        if (
